...
def convert_nfo_to_html(nfo_filepath, output_filepath):
    ...
    year = root.findtext('year', 'N/A')
    rating = root.findtext('rating', 'N/A')
...
<section class="w3-container w3-black">
    ...
    <div class="w3-container w3-black w3-cell"><img src="./folder.jpg" width="400" heigth="600" Alt="Capa"></div>
    <div class="w3-container w3-black w3-cell">
            <p>{plot}</p>
            <p><b>Título Original:</b> {originaltitle}</p>
            <p><b>Estréia:</b> {premiered}</p>
            <p><b>Ano:</b> {year}</p>
            <p><b>Nota:</b> {rating}</p>
            <p><b>Clasificação:</b> {mpaa} Anos</p>
            <p><b>Estudios:</b> {', '.join(studios) if studios else 'N/A'}</p>
            <p><b>Gêneros:</b> {', '.join(genres) if genres else 'N/A'}</p>
            <p><b>Tags:</b> {', '.join(tags) if tags else 'N/A'}</p>
...
"""
    ...
    <footer class="w3-center">
        <p>&copy; 2025 TV Show Converter.</p>
    </footer>
    </section>
    </main>
